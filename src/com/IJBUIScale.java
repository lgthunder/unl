...
public class IJBUIScale {
    ...
    private static final PropertyChangeSupport PROPERTY_CHANGE_SUPPORT = new PropertyChangeSupport(new IJBUIScale());
    ...
    private static final MutableNotNullValue<Float> SYSTEM_SCALE_FACTOR = new MutableNotNullValue<>(() -> {
        ...
        if (JreHiDpiUtil.isJreHiDPIEnabled()) {
            ...
            if (gd != null && gd.getDefaultConfiguration() != null) {
                return Float.valueOf(sysScale(gd.getDefaultConfiguration()));
            }
            return Float.valueOf(1.0F);
        }

        float result = getFontScale(((Integer)((Pair)systemFontData.getValue()).getSecond()).intValue());
